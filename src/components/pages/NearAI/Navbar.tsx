...
const NavLink = ({ href, children, external, alpha, alphaV2, onClick }: NavLinkProps) => {
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-2 text-[#AFD0C5] hover:text-white px-4 py-2"
    ...
    >
      <span className="flex items-center gap-1">
        {children}
        {alpha && <AlphaLabel />}
        {alphaV2 && <AlphaLabel version="v2" />}
      </span>
      {external && <span className="w-4" />}
    </Link>
  );
};
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <Link href="/" className="text-2xl font-bold">
              <img src="/logo-white.svg" alt="NEAR AI" className="h-[18px]" />
            </Link>
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <a
                ...
                className="p-2 text-[#AFD0C5] hover:text-white"
